...
class ModelMutationOperatorsUtils():
    ...
    def LD_model_scan(self, model):
        ...
        for index, layer in enumerate(layers):
            ...
            # if layer_name=="Add":#针对resnet的add层
                # continue
    ...
    def LAm_model_scan(self, model):
        ...
        for index, layer in enumerate(layers):
            ...
            # if layer_type_name=="Add":#针对resnet的add层
            #     continue
    ...
    def AFRm_model_scan(self, model):
        ...
        layers = [l for l in model.layers]
        for index, layer in enumerate(layers):
            if index == (len(model.layers) - 1):#不选择最后一岑更多激活函数
                continue
            try:
...
class ModelMutationOperators():
    ...
    def LD_mut(self, model, mutated_layer_indices=None):
        ...
        # print(number_of_suitable_layers)
        ...
        if mutated_layer_indices == None:
            ...
            for index, layer in enumerate(layers):
                if index == random_picked_layer_index:
                    continue
                # if type(layer).__name__ == "Add":  # 针对resnet的add层
                #     print(layer.input.shape.as_list(),layer.output.shape.as_list())
                new_model.add(layer)
        else:
        ...
        if mutated_layer_indices == None:
            ...
            for index, layer in enumerate(layers):
                if index == random_picked_layer_index:
                    ...
                    new_model.add(layer)
                    continue
                # print("name:",type(layer).__name__," :: ",layer.input.shape.as_list()," :: ",layer.output.shape.as_list())
                new_model.add(layer)
        else:
            self.check.in_suitable_indices_check(index_of_suitable_layers, mutated_layer_indices)
            for index, layer in enumerate(layers):
